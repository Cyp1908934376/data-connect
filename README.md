...
- **可视化模板** — 用步骤编排取数、映射、写库、下载、归档推送，支持立即执行与定时
- **灵活的任务调度** — 可视化模板选 Cron 后自动出现在任务列表
- **接口调用** — SINGLE（单次调用）、CHAIN（链式调用）
...
用事件步骤编排对接：数据源 → 映射/过滤/操作 → 输出（返回、调用子模板、写出文件）。支持分批、增量水位、Cron 定时、立即执行与运行日志。
